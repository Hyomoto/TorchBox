...
class ColorSpan:
  ...
  def __repr__(self):
    return f"{self.color}{self.start}-{self.end}{Ansi.RESET}"
...
class ColorCode:
  ...
  def __repr__(self):
    return f"{self.code}@{self.position}{Ansi.RESET}"
...
class Canvas:
    ...
    def draw(self, sprite: Sprite, x: int = None, y: int = None, color = None):
        for i, line in enumerate(sprite.lines):
            ...
            self.write(line, x=x, y=y, color = color)
            y = self.cursor[1] + 1
        ...
        def applyColorSpans(x, spans: List[ColorSpan], table: List[ColorCode]):
            ...
            while spanLast:
                if tableIndex < len(table) and table[tableIndex].position < spanLast.start + x:
                    ...
                    table.insert(tableIndex, ColorCode(spanLast.end + x, Ansi.RESET))
                    spanIndex += 1
    ...
    def line(self, x0, y0, x1, y1, **kwargs):
        ...
        if isinstance(pattern, str):
            ...
            left, mid, right = pattern[0], pattern[0], pattern[0]
        elif len(pattern) == 3:
    ...
    def box(self, x, y, width, height, outline=True, color=None, pattern=("█",)):
        ...
        # Draw to canvas
        for i, line in enumerate(lines):
            self.write(line, x=x, y=y + i, color=color)
    ...
    def render(self):
        ...
        for i, colors in enumerate(self.colors):
            ...
            stack = []
            for color in colors:
                if color.code == Ansi.RESET:
                    if stack:
                        stack.pop()
                    color.code = stack[-1].code if stack else Ansi.RESET
                else:
                    stack.append(color)
